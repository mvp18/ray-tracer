...
void trynomapper(const char* filename) {
    ...
    ImageTexture* whitetex    = new ImageTexture("models/assignments/stones_diffuse.png");
...
void trymapper(const char* filename, CoordMapper* spheremapper1, CoordMapper* spheremapper2) {
    ...
    ImageTexture* whitetex = new ImageTexture("models/assignments/stones_diffuse.png");
    ImageTexture* clamptex = new ImageTexture("models/assignments/stones_diffuse.png", ImageTexture::CLAMP);
    ImageTexture* mirrtex = new ImageTexture("models/assignments/stones_diffuse.png", ImageTexture::MIRROR);
...
void tryenvmapper(const char* filename, CoordMapper* spheremapper1, CoordMapper* spheremapper2) {
    ...
    world.light.push_back(new AmbientLight(RGBColor(40000.0f*scale*scale, 0.0f, 0.0f)));
    ...
    //remove back wall and add environment solid
    ImageTexture* envtex = new ImageTexture("models/assignments/night_sky_high_res.png", ImageTexture::MIRROR, ImageTexture::BILINEAR);
    Material* envmat = new FlatMaterial(envtex);
    scene.add(new EnvironmentSolid(new EnvironCoordMapper(), envmat));
...
void a_mappers() {
    ...
    // tryenvmapper("mapper-environment.exr",
    //     new CylindricalCoordMapper(Point(.4f,.45f,.3f),Vector(0.0f,hsq2,hsq2),Vector(0.5f,0.0f,0.0f)),
    //     new CylindricalCoordMapper(Point(.3f,.1f,.3f),Vector(0.0f,hsq2,-hsq2),Vector(0.5f,0.0f,0.0f))
    // );

    tryenvmapper("mapper-environment-ambient.exr",
        new CylindricalCoordMapper(Point(.4f,.45f,.3f),Vector(0.0f,hsq2,hsq2),Vector(0.5f,0.0f,0.0f)),
        new CylindricalCoordMapper(Point(.3f,.1f,.3f),Vector(0.0f,hsq2,-hsq2),Vector(0.5f,0.0f,0.0f))
    );
}
